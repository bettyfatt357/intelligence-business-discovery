...
export class EmailQueue {
  ...
  async markCompleted(
    ...
    if (job) {
      ...
      // Phase 2D: Store intelligence extraction results if provided
      if (intelligence) {
        job.intelligence = {
          company: intelligence.company,
          emails: intelligence.emails,
          pageMetadata: intelligence.metadata,
          matchedPatterns: intelligence.extractionMethods?.map((method) => ({
            pattern: method,
            stage: 'html' as const,
            confidence: 0.8,
          })),
      ...
      console.log(
        `[QUEUE] Job completed: ${jobId} - found ${emails.length} emails in ${job.processingTime}ms${
          intelligence ? ` - quality: ${intelligence.qualityScore.toFixed(2)}` : ''
        }`
      );
    }
  }
